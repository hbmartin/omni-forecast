...
per-variable truth semantics all match — deliberately not the dataset
fingerprint, which rotates on every ingested observation and would otherwise
leave long leads with no scoreable evidence. The cold-start fallback is fit-free
